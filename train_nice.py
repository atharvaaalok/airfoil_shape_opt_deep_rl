import argparse
import os
from pathlib import Path
import random
import sys

from geodiff.utils import sample_T
import numpy as np
from stable_baselines3 import PPO
from stable_baselines3.common.vec_env import SubprocVecEnv
from stable_baselines3.common.vec_env import VecMonitor
from stable_baselines3.common.logger import configure
import torch
from torch import nn
from torch.nn.utils import parameters_to_vector, vector_to_parameters

from src.shape_assets.nice_normalized import NiceNorm_Airfoil
from src.cfd_utils.aerodynamics import compute_L_by_D
from src.gym_envs.nice_shape_env import NiceShapeEnv
from src.shape_assets.utils import plot_curves


# Set random seed for reproducibility
def set_global_seed(seed: int) -> None:
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)


base_log_dir = Path('logs')


# Specify gym environment properties
airf_to_airf = True
nice_small = True
config = {
    'action_sigma': 0.001,
    'max_episode_steps': 8,
    'non_convergence_reward': -50.0,
    'num_env': 16,
    'total_training_timesteps': 1_000,
    'n_steps_per_env': 8,
    'batch_size': 64,
    'Mach_num': 0.1,
    'Re': 1e6,
    'nice_model': f'nice_{"airfoil_" if airf_to_airf else ""}normalized_fit_to_naca0010{"_small" if nice_small else ""}.pth',
    'policy_learning_rate': 0.0003,
    'NICE_class': NiceNorm_Airfoil,
    'policy_kwargs': dict(net_arch = [64, 64]),
    'xfoil_max_iter': 250,
    'gamma': 1.0,
    'reward_delta': False,
    'nice_layer_count': 3 if not nice_small else 1,
    'nice_coupling_net_hidden_dim': 10 if not nice_small else 5,
}

NICE_class = config['NICE_class']


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--seed', type = int, default = 0, help = 'Random seed for reproducibility')
    parser.add_argument('--log-name', type = str, default = 'default_log',
                        help = 'Name of the log directory under logs/.')
    args = parser.parse_args()
    seed = args.seed
    log_name = args.log_name

    # Set seed and log name for the experiment
    set_global_seed(seed)
    log_path = base_log_dir / log_name / f'seed_{seed}'


    print('Configuration:\n' + '-'*50)
    for config_key, config_value in config.items():
        print(f'{config_key} = {config_value}')

    # Import the NICE model that we trained to fit the airfoil
    nice = NICE_class(
        layer_count = config['nice_layer_count'],
        coupling_net_layer_count = 1,
        coupling_net_hidden_dim = config['nice_coupling_net_hidden_dim'],
        use_batchnormalization = False,
        use_residual_connection = False,
        volume_preserving = False
    )
    nice.load_state_dict(torch.load(
        Path(f'./src/shape_assets/models/{config['nice_model']}'), weights_only = True
    ))

    # Print the model parameters to get an idea of the scale of values
    print('\nNICE Parameters:\n' + '-'*50)
    print(f'nice_model = {config['nice_model']}')
    for name, param in nice.named_parameters():
        print(f"\n{name}:\n{param}")


    print('\nCreating Environment:\n' + '-'*50)
    # Create a gym environment
    def make_nice_env(rank: int, seed: int = 0):
        def _init():
            env = NiceShapeEnv(
                nice = nice,
                action_sigma = config['action_sigma'],
                max_episode_steps = config['max_episode_steps'],
                non_convergence_reward = config['non_convergence_reward'],
                Mach_num = config['Mach_num'],
                Re = config['Re'],
                xfoil_max_iter = config['xfoil_max_iter'],
                reward_delta = config['reward_delta']
            )
            env.reset(seed = seed + rank)
            return env

        return _init

    # Create vectorized gym environments
    train_env = SubprocVecEnv(
        [make_nice_env(rank = i, seed = seed) for i in range(config['num_env'])]
    )
    train_env = VecMonitor(train_env)


    # Setup logging
    tensorboard_log_file_location = log_path
    # Setup logger
    training_logger = configure(str(tensorboard_log_file_location),
                                ['stdout', 'tensorboard', 'log'])


    # Create RL model
    model = PPO(policy = 'MlpPolicy', learning_rate = config['policy_learning_rate'],
                env = train_env, verbose = 1, tensorboard_log = tensorboard_log_file_location,
                n_steps = config['n_steps_per_env'], batch_size = config['batch_size'], seed = seed,
                policy_kwargs = config['policy_kwargs'], gamma = config['gamma'])
    
    model.set_logger(training_logger)


    print('\nModel Training:\n' + '-'*50)
    # Start training
    model.learn(total_timesteps = config['total_training_timesteps'], progress_bar = True)


    print('\nModel Evaluation:\n' + '-'*50)
    # Policy evaluation
    test_rank = 100
    test_env_init = make_nice_env(rank = test_rank, seed = seed)
    test_env = test_env_init()
    observation, info = test_env.reset()
    done = False

    print(f'{"Step":>4} {"L_by_D":>10}')
    step = 0
    while not done:
        step += 1
        # Get action from the trained policy
        action, _state = model.predict(observation, deterministic = True)

        # Step the environment
        observation, reward, terminated, truncated, info = test_env.step(action)
        done = truncated
        print(f'{step:4d} {reward:10.4f}')


    # Convert observation to NICE parameters
    test_nice = NICE_class(
        layer_count = config['nice_layer_count'],
        coupling_net_layer_count = 1,
        coupling_net_hidden_dim = config['nice_coupling_net_hidden_dim'],
        use_batchnormalization = False,
        use_residual_connection = False,
        volume_preserving = False
    )
    vector_to_parameters(torch.from_numpy(observation), test_nice.parameters())

    # Calculate the L-by-D ratio of the final shape and plot the airfoil produced
    num_pts = 251
    T = sample_T(geometry_dim = 2, num_pts = num_pts)
    X_test = test_nice(T)
    L_by_D = compute_L_by_D(X = X_test.detach().cpu().numpy(), M = config['Mach_num'],
                            Re = config['Re'])

    X_original = nice(T)
    fig_filename = log_path / 'final_shape.svg'
    plot_curves(X_original, X_test, filename = fig_filename, show_plot = False)
    L_by_D = f'{L_by_D:7.3f}' if L_by_D is not None else str(L_by_D)
    print(f'\n\nFinal L_by_D of trained policy: {L_by_D}')
    print(f'Last reward: {reward:7.3f}')